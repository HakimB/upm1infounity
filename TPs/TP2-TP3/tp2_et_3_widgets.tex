...
\begin{solution}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Préciser les dimensions, par exemple\texttt{ Width = 990} et \texttt{Height = 100}.
		\end{itemize}
	\item Dans  \texttt{PanelInputFieldText}:
	\begin{itemize}
		\item Ajouter un \texttt{ UI (Canvas) > Legacy > Input Field} et le renommer \textit{InputFieldText1} et modifier son \texttt{Rect Transform}:
		\begin{itemize}
...
{
	// Attention : si les attributs sont classes "public",
	// ils apparaitront dans l'Inspector. Mais s'ils ne sont pas initialises
	// dans le code, l'Inspector peut leur affecter
	// à l'exécution une valeur quelconque (vide par exemple).
	// Et c'est cette valeur qui sera prise en compte.
	
	// Regex validant uniquement une suite de chiffres.
	// Modifiable dans l'Inspector.
	// Il faut initialiser cet attribut ou il recevra la valeur ""vide" 
...
\begin{csharpsansbord}
	...
	private void ValueChangeCheck() {
		...
		if (m_Text == null) {
			...
			// Le type "string" ne supporte pas la concatenation comme Java
...
\begin{itemize}
	...
	\begin{itemize}
		\item un  \texttt{Text avec UI (Canvas) > Legacy > Text} renommé \textit{Text}
		\begin{itemize}
		    ...
		    \begin{itemize}
		    	\item \texttt{Text = x}
		    	\item \texttt{Font Size = 36} [\textbf{ATTENTION}: si la taille est trop grande, rien n'apparaît dans le widget]
		    	\item \texttt{Paragraph > Alignment = middle / center}
		    \end{itemize}
		\end{itemize}
		\item un \texttt{Slider} avec \texttt{UI (Canvas) > Slider}
		\begin{itemize}
			\item \texttt{Rect Transform}
			\begin{itemize}
				\item \texttt{Anchor = bottom left}
		...
		\end{itemize}
		\item un \texttt{Input Field} \textit{InputFieldComplexSlider} avec \texttt{UI (Canvas) > Legacy > Input Field}
		\begin{itemize}
			\item \texttt{Rect Transform}
			\begin{itemize}
				\item \texttt{Anchor = bottom left}
...
\subsection{Script \textit{ComplexSliderScript.cs} à placer dans PanelComplexSlider1}
...
Il suffit de sélectionner le composant qui nous intéresse : ici, \textit{PanelComplexslider1}, et de le déplacer dans le \texttt{Project > Assets}.\\
...
Remarque : si l'on sélectionne ce \texttt{Prefab}, l'\texttt{Inspector} affiche un bouton \texttt{Open} (et une flèche '\texttt{>}' à droite du nom du \texttt{Prefab}) qui modifie l'interface principale pour lister uniquement la hiérarchie de ce \texttt{Prefab}. Pour revenir à la hiérarchie complète, on clique sur le symbole '\texttt{<}' à gauche du nom du \texttt{Prefab} dans la hiérarchie.
...
\begin{itemize}
	...
	\item clique droit : sélectionner \texttt{Export as Asset Package} et ne cocher que les dépendances nécessaires:
	\begin{itemize}
...
\begin{solution}
	\begin{itemize}
		...
		\item Sélectionner \textit{MyPrefabComplexslider.unitypackage} (on peut sélectionner tout ou partie du package)
...
\ifversionenseignant
\begin{solution}
	\href{ https://docs.unity3d.com/Packages/com.unity.ugui@2.5/manual/HOWTO-UIMultiResolution.html}{Documentation multiresolution}. \\
...
À partir du lien \href{https://docs.unity3d.com/Packages/com.unity.ugui@2.5/manual/UICanvas.html}{UICanvas}, expérimenter les modes de rendu "\texttt{Screen Space - Camera}" et "\texttt{World Space}" pour modifier l'orientation et la position du \texttt{Canvas} et de ses enfants (fig.~ci-dessous). Noter que la caméra associée devra peut-être être mise à jour pour visualiser le \texttt{Canvas}.
