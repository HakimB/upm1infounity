...
\begin{enumerate}
	\item Installer  \texttt{UnityHub} et la dernière version de \texttt{Unity}. La version sur laquelle ce tutorial a été réalisée est : \texttt{UnityHub 3.16.3} et \texttt{Unity 6.3 LTS (6000.3.7f1)}.
	\item  Ouvrir \texttt{UnityHub} et sélectionner à partir du bandeau \texttt{Projects}:
	\begin{itemize}
		\item "\texttt{New Project}"
		\item "\texttt{Universal 3D Core}"
		\item "\texttt{Project name}" $\rightarrow$ cliquer sur le champ \texttt{My project (1)} puis"\texttt{Create new local project}" et entrer "\textit{TP1 Solar System}"
...
\begin{itemize}
	\item chercher sur un site libre un objet 3D au format \texttt{OBJ}, par exemple sur le site \href{https://turbosquid.com/}{TurboSquid}\footnote{Nécessite une inscription gratuite} (mot-clé: "\textit{rock}" avec filtre "\textit{price = free}"). Une fois l'archive téléchargée, il faut l'ouvrir puis glisser-déposer le dossier correspondant dans \texttt{Assets > Images};
	\item installer un plugin spécifique pour reconnaître le format \texttt{GLTF} (solution toujours effective en 2026):
	\begin{itemize}
		\item Dans l'interface, ouvrir \texttt{Window > Package Manager} (si ce dernier propose des mises à jour des packages pré-installés, valider ces dernières -- chacune peut prendre du temps).
...
\begin{solution}
\paragraph{Material personnalisé}

\begin{itemize}
	\item Dans l'onglet \texttt{Project}, bouton droit de la souris sur le dossier \texttt{Assets} ; créer l'arborescence de dossiers \texttt{Assets > Images > Materials}.
	\item Dans une zone libre de \texttt{Images} (qui devrait contenir des images fournies avec le TP)
	\begin{itemize}
		\item clic droit pour créer un nouveau \textit{Material} portant le nom d'une image, par exemple \textit{2k\_earth\_daymap} (ne pas préciser l'extension, \textit{.mat} sera ajouté ensuite),
		\item faire un drag'n drop de l'image vers le petit carré à gauche du champ \texttt{Surface Inputs > Base Map} du nouveau \texttt{Material}
		\item[$\rightarrow$] l'image apparaît en miniature à côte de \texttt{Base Map} et modifie le rendu du \texttt{Material}. 	
	\end{itemize}
	\item Déplacer le \texttt{Material} dans le dossier \texttt{Assets > Images > Materials}.
	\item Répéter l'opération pour associer un \texttt{Material} à chaque image.
\end{itemize}

\paragraph{Création de la SkyBox}

\begin{itemize}
	
	\item Sélectionner la \texttt{Main Camera} de la scène.
	
\item Cliquer sur le bouton \textit{Add Component} en bas de l'\texttt{Inspector}. Dans la barre de recherche, entrer \texttt{Skybox}.



	\item Sélectionner le \texttt{Material} nommé	\textit{8k\_stars\_milky-way.mat}.
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}				
				...
				\item \texttt{Shadows > Shadow Type}: \texttt{Soft shadows}
			\end{itemize}
		\end{itemize}
		...
		\begin{itemize}
			...
			\item \texttt{Emission > Emission Map}: la même texture (si le drag'n drop ne fonctionne pas, cliquer sur le petit cercle juste à gauche de \texttt{Emission Map} ouvre une fenêtre permettant de sélectionner la texture)
			\item Vérifier que la case à côté de \texttt{Emission Map} est cochée
			\item Cliquer sur le champ \texttt{HDR} et dans le carré à l'intérieur du cercle de couleur, diriger la souris vers le coin blanc en haut à gauche pour rendre l'aspect plus ou moins brillant).
		\end{itemize}
...
Le résultat devrait ressembler à la fig.~\ref{fig:config-soleil} (en sélectionnant l'onglet \texttt{Game}). Il peut être utile de déplacer la caméra.
\end{solution}
...
\item \texttt{Ìnspector > Shader > Advanced Options} : activer \texttt{Enable GPU instancing} 
\begin{itemize} 
...
Reprendre la procédure précédente pour intégrer \textit{Deimos} comme lune de \textit{Mars}. On peut utiliser le fichier \href{https://science.nasa.gov/resource/deimos-mars-moon-3d-model/}{sur ce site}.
...
public class JeTourne : MonoBehaviour
{
	...
	// sous forme de [Toggle] dans les proprietes du script de
	// l'Inspector, sous le nom [Must Turn]
	// Modifier cette valeur dans l'Inspector modifie cet attribut 
	// dans ce code
...
\textbf{Attention} : sous Linux, la recompilation des scripts n'est pas automatique. Plusieurs solutions sont proposées \href{https://discussions.unity.com/t/force-unity-to-recompile-scripts/65762/9}{ici}, notamment~:\\
Menu principal : \texttt{Assets > Refresh}
...
\begin{itemize}
	\item Dans \texttt{Hierarchy} : bouton droit : \texttt{UI (Canvas) > Canvas}. Un objet \texttt{EventSystem} est également automatiquement ajouté.
...
\begin{itemize}
	\item Ajouter un \texttt{UI  (Canvas) > Panel} nommé \textit{PanelSoleil} comme enfant de \textit{Canvas}. 
...
\begin{itemize}
	\item Ajouter un \texttt{UI (Canvas) > Legacy > Text} nommé "\textit{NomPanelSoleil}" comme enfant de \textit{PanelSoleil}.
	\item \texttt{Inspector > Rect Transform}
	\begin{itemize}
...
\begin{itemize}
	\item Ajouter un \texttt{UI (Canvas) > Toggle} nommé "\textit{TogglePanelSoleil}" comme enfant de \textit{PanelSoleil}.
	\item \texttt{Inspector > Rect Transform}
	\begin{itemize}
		\item \texttt{Stretch / Stretch > top / left}
		\item \texttt{Pivot X = 0 ; Y = 1}
...
\begin{itemize}
	\item Ajouter un \texttt{UI (Canvas) > Legacy > Text} nommé "\textit{VitesseAnimSoleil}" comme enfant de \textit{PanelSoleil}.
	\item \texttt{Inspector > Rect Transform}
	\begin{itemize}
		\item \texttt{Stretch / Stretch > top / left}
		\item \texttt{Pivot X = 0 ; Y = 1}
...
\begin{itemize}
	\item Ajouter un \texttt{UI (Canvas) > Slider} nommé "\textit{SliderVitesseAnimSoleil}" comme enfant de \textit{PanelSoleil}.
	\item \texttt{Inspector > Rect Transform}
	\begin{itemize}
		\item \texttt{Stretch / Stretch > top / left}
		\item \texttt{Pivot X = 0 ; Y = 1}
...
\begin{itemize}
	\item Dupliquer \textit{PanelSoleil} avec \texttt{clic droit -> Duplicate} dans la \texttt{Hierarchy} en deux exemplaires : \textit{PanelTerre} et \textit{PanelMars}
...
\begin{itemize}
	...
	\item Ne pas oublier de passer la \texttt{Max Value} de \texttt{SliderVitesseAnimTerre} et \texttt{SliderVitesseAnimMars} à $100$ si nécessaire.
\end{itemize}
...
\begin{itemize}
	\item Lorsque plusieurs caméras sont utilisées, la caméra visible est celle dont la valeur de la propriété \texttt{Renderer > Priority} est la plus élevée.
...
La mécanique céleste mise en place jusqu'à présent est vraiment basique, mais offre un cadre suffisant pour l'apprentissage des animations. Nous vous proposons d'ajouter une comète cyclique (ou autre astre) qui n'est plus enfant du Soleil, mais bien un objet quelconque.
...
\begin{itemize}
	\item Importer un objet 3D au format \texttt{.obj} pour représenter un nouvel astéroïde, par exemple \href{https://nasa3d.arc.nasa.gov/detail/bennu}{Bennu}\footnote{Ce maillage n'a pas de normales incluses, mais \texttt{Unity} les recalcule automatiquement.}.
	...
	\item Associer à cet objet  un \texttt{Effects > Trail}. 
...
Le script suivant \texttt{TrajectoireScript.cs} associé à \textit{Bennu} permet de créer une trajectoire elliptique autour du centre du repère global, avec le soleil constituant l'un des foyers de l'ellipse. L'attribut \textit{semiMinor}  de \texttt{Bennu} accessible depuis l'\texttt{Inspector} permet de changer la forme de l'ellipse. \\
...
\begin{boxcode}
	\begin{csharpsansbord}
// Assets > Editor > ShowOnlyDrawer.cs
// Source: https://discussions.unity.com/t/how-to-make-a-readonly-property-in-inspector/75448/4
...
Remarque: le \texttt{trail} se comporte parfois bizarrement quand on modifie la forme de l'ellipse: l'objet suit bien la nouvelle trajectoire mais le \texttt{trail} conserve toujours la même (?).
\newpage 
\fi 
...
Sélectionner l'ensemble des \texttt{Toggle} enfants, puis cliquer sur \texttt{ToggleGroupLookAt} et le déplacer à la souris dans la propriété \texttt{Toggle > Group} des enfants sélectionnés (ou bien cliquer sur le petit cercle à côté de cette propriété et sélectionner \texttt{ToggleGroupLookAt} dans la liste). La valeur de cette propriété passe de \texttt{None (Toggle Group)} à \texttt{ToggleGroupLookAt (Toggle Group)}.\\
