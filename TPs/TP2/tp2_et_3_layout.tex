...
L'objectif de ce widget est de réaliser une zone de texte qui change de couleur selon une expression régulière particulière comme sur l'image ci-dessous, où le champ valide la présence d'un nombre, ainsi lorsque la saisie est vide, nous avons une couleur standard, la présence d'un nombre engendre une couleur et une autre couleur si l'expression régulière n'est pas présente.
\begin{center}
	\includegraphics[width=0.4\linewidth]{rc/widget_formattetinputfield_start}
\end{center}

Pour cela, nous vous donnons le code suivant
...
if (System.Text.RegularExpressions.Regex.IsMatch(regex, montext))
...
Pour cela, il faut exporter des Préfabs dont vous connaissez les dépendances, en sélectionnant votre Préfab dans les Assets et en faisant un clique droit dessus. Dans le menu contextuel, sélectionnez \texttt{'Export Package\ldots'}.
